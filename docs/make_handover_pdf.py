...
⚠️ 관리자 비밀번호는 이 문서에 넣지 않습니다.
   문서는 메일·메신저로 오가는데, 비밀번호가 함께 남으면 유출 경로가 됩니다.
   비밀번호는 교육 때 구두로 전달하고 협회가 즉시 변경하도록 안내합니다.
"""
...
a(table([
    ...
    [C("교육"), C("CMS 사용법 안내 1회"), C("일정 협의 예정")],
...
a(table([
    ...
    [CB("관리자 아이디"), C("100kgo")],
    [CB("관리자 비밀번호"), C("교육 시 별도 전달 (보안상 문서에 기재하지 않습니다)")],
], [30 * mm, 135 * mm]))
a(Spacer(1, 2 * mm))
a(P("• 관리자 화면 주소는 홈페이지 어디에도 링크되어 있지 않으며 검색에도 노출되지 않습니다. "
    "즐겨찾기에 등록해 두시기 바랍니다.<br/>"
    "• <b>전달받으신 초기 비밀번호는 교육 직후 협회에서 직접 변경하시기를 권합니다.</b>", st_note))
...
a(Spacer(1, 2 * mm))
a(P("사용법은 별도 문서 「협회 홈페이지 게시판 사용법」에 화면별로 정리되어 있습니다.", st_note))
...
a(table([
    [CB("항목"), CB("내용")],
    [C("관리자 비밀번호"), C("교육 직후 변경하시고, 담당자 변경 시 함께 변경해 주세요.")],
